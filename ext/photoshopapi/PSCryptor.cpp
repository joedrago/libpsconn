...
// [jdrago 2018/10/16] - there are 3 signed/unsigned mismatches in this file, shh
...
HCRYPTKEY GetSessionKey(const u_int8_t *key)
{
    ...
    if (!CreatePrivateExponentOneKey(MS_ENHANCED_PROV, PROV_RSA_FULL,
                                     "Adobe Photoshop", AT_KEYEXCHANGE, // [jdrago 2018/10/16] - I'm building MBCS, so I removed the wide specifier on the literal string
                                     &hProv, &hPubPrivKey))
	{
		return 0;
	}
